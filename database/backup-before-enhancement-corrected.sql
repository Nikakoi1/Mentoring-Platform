-- CORRECTED Backup Script for Admin Reports Enhancement
...
-- Note: Your schema uses 'pairings' not 'mentor_pairings'
CREATE TABLE IF NOT EXISTS backup_pairings AS
...
CREATE TABLE IF NOT EXISTS backup_client_visits AS
SELECT * FROM public.client_visits;

CREATE TABLE IF NOT EXISTS backup_client_visit_evaluations AS
SELECT * FROM public.client_visit_evaluations;
...
SELECT 
  'backup_pairings' as table_name,
  COUNT(*) as row_count
FROM backup_pairings
...
SELECT 
  'backup_resources' as table_name,
  COUNT(*) as row_count
